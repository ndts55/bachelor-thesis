...
Chandra and Toueg introduce a class of failure detectors $\FailureDetectorClass$, which is called \emph{eventually strong} in \cite{failure_detectors}.
...
Let $\SetOfPermanentlySuspectedProcesses$ be the set of permanently suspected processes. For proposers $\FPcrash$ returns $\True$ if $\AQ\setminus\SetOfPermanentlySuspectedProcesses$ is not a quorum, i.e. if the set of acceptors in $\AQ$ that are not permanently suspected is not enough to form a majority of acceptors.
