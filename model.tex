...
\\
&\PpInit{\ProposerIndex}{\AQ}{n}{m}{\VectorV} = \SessionRequest{\SharedPoint{n}}{\ProposerIndex}{s} . \Pp{}\\
...
Each proposer has the same role $\ProposerIndex = \AcceptorCount + 1$ but uses a different shared channel $\SharedPoint{n}$ according to its index $n$.
...
Shared channel $\SharedPoint{n}$ is used to initiate a session.
...
\begin{align*}
&\Pp{} = \Mu{\RecursionVariable} \update{n}{n + 1} .\\
...
&\Indent{3}\DotForall{\AcceptorIndex\in\AQ}{\SendUnreliableP{s}{\ProposerIndex}{\AcceptorIndex}{\LTwoA}{\ProposalC{\Big ( \proposalNumber{n}{m} \Big )}{\Paren{\promiseValue{\VectorV}}}}}.\End
}}
\end{align*}
...
&\Indent{2}\Else{\big( \If{\greaterThan{n'}{n}}\\
...
For the weakly reliable broadcast in phase $\PTwoA$, the failure pattern $\FPwskip$ returns $\True$ for subprocesses of acceptors if the corresponding proposer crashed, otherwise terminated, or the corresponding proposer's quorum does not include that particular acceptor.
...
For acceptors $\FPcrash$ returns $\True$ if at least one more acceptor may crash, i.e. $n > 2(f + 1)$ is satisfied.
...
% TODO These failure patterns satisfy conditions 1.1-1.6
...
\subsection{Formulae}
We set $\AcceptorCount = 3$, $\ProposerCount = 2$, and $\Value=\Curly{\ABC, \operatorname{def}, \dots, \operatorname{vwx}, \operatorname{yz}}$.
...
&\Mu{\RecursionVariable} \update{n}{5} . \DotForall{\AcceptorIndex\in \{1,2\}}{\SendUnreliableP{s}{4}{\AcceptorIndex}{\LOneA}{\proposalNumber{n}{4}}}\dots \comment{= \Proposer{4}}\\
&\Or \Mu{\RecursionVariable} \update{n}{6} . \DotForall{\AcceptorIndex\in \{2,3\}}{\SendUnreliableP{r}{4}{\AcceptorIndex}{\LOneA}{\proposalNumber{n}{5}}}\dots \comment{= \Proposer{5}}\\
&\Or \Paren{
