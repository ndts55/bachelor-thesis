\chapter{Model}
\label{chapter:model}
First, we specify some sorts that are used to define the global type.
...
In step $\PTwoA$ the proposer decides whether to send an $\Accept$ or $\Restart$ message.
This decision is broadcast to all roles in $\QRoles$.
After sending an $\Accept$ message the proposer sends its proposal to all roles in $\QRoles$ and terminates.
By sending a $\Restart$ message the proposer restarts the algorithm.
...
\begin{enumerate}
    \item $\forall p,q \in \SetOfProposers . \forall n,m \in \mathbb{N}: p \neq q \to \proposalNumber{n}{p} \neq \proposalNumber{m}{q}$
...
\\
    \item $\forall p \in \SetOfProposers . \forall n,m \in \mathbb{N}: n > m \to \proposalNumber{n}{p} > \proposalNumber{m}{p}$
...
In Paxos a majority of acceptors forms a quorum, \ie an accepting set that can choose a value \cite{Lamport06}.
We use this function to define a quorum corresponding to a proposer.
The proposer then communicates with the acceptors in its quorum.
...
\section{Processes}
In this section we define the processes to model Paxos.
We start with the system initialization, which contains the session initialization and starts the proposers and acceptors with the correct arguments.
Next, we define the proposer and acceptor processes, which model the Paxos algorithm itself.
...
\RegisterContent{$\Just{15}$}{$\Just{15}$}{$\Just{10}$}{$2$}{$1$}{$\Just{\Paren{\ProposalC{15}{\ABC}}}$}{$\Just{\Paren{\ProposalC{15}{\ABC}}}$}{$\Just{\Paren{\ProposalC{10}{\ABC}}}$}
