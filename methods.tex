...
% TODO define failure patterns
% we don't need to reject outdated messages
% thus fpuget = true
In Paxos there is no need to reject outdated messages so $\FPuget$ is implemented with a constant $\True$.

$\FPuskip$
% TODO ask KP about it
% eventually weak? eventually strong?

$\FPwskip$
% only iff corresponding proposer crashed? eventually weak? eventually strong?

$\FPml$
% same as uskip

$\FPcrash$
% a majority of acceptors still needs to be correct -> n > 2f
