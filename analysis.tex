...
Acceptors terminate if all of their sub-processes terminate.
Each sub-process of an acceptor corresponds to one proposer.
A sub-process can only terminate via the weakly reliable broadcast in $\PaTwo$, which depends on the corresponding proposer.
If that proposer crashes or its quorum does not include the acceptor, the sub-process terminates because $\FPwskip$ returns $\True$ and the default branch is $\Abort$, which terminates immediately.
The termination of a sub-process with a correct proposer requires the termination of that proposer.
...
In \cite{lamport2001paxos} \citeauthor{lamport2001paxos} describes a scenario in which two proposers loop endlessly, never having their proposals accepted:
...
From \cite{lamport2001paxos} we know that this problem is solved by electing a single distinguished proposer to be the leader.
